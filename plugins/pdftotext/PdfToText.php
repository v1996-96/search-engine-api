	...
	protected function  IsPageHeaderOrFooter ( $stream_data )
	   {
		if  ( preg_match ( '#/Type \s* /Pagination \s* /Subtype \s*/((Header)|(Footer))#ix', $stream_data ) )
			return ( true ) ;
		else if  ( preg_match ( '#/Attached \s* \[ .*? /Top [^]]#ix', $stream_data ) )
			return ( true ) ;
		else
			return ( false ) ;
	    }
...
class  PdfToText 	extends PdfObjectBase
   {
	...
	// A subset of a character classification array that avoids to many calls to the ctype_* functions or too many
								...
								for  ( $i = 1, $length = strlen ( $text ) - 1 ; $i  <  $length ; $i ++ )
								   {
									...
									if  ( ord ( $ch )  <  127 )
										$newchar	=  $ch ;
									else if  ( isset  ( $this -> CharacterMapBuffer [ $current_font ] [ $ch ] ) )
										$newchar	=  $this -> CharacterMapBuffer [ $current_font ] [ $ch ] ;
									else
									   {
										$newchar	=  $this -> FontTable -> MapCharacter ( $current_font, ord ( $ch ) ) ;
		...
		while  ( ( $part = $this -> __next_token ( $data, $data_length, $index ) )  !==  false )
		   {
			...
			// Token starts with a slash : maybe a font specification
			else if  ( $token [0]  ==  '/'  &&  isset ( $token [1] )  &&  isset ( $token [2] ) )
			   {
				...
				switch  ( $ch )
				   {
					// "/R" : specifies directly a font object describing the font
					case	'R' : case 'r' :
	...
	public function  AddFontMap ( $object_id, $object_data )
	   {
		...
		if  ( preg_match_all ( '# (?P<font> / ( (F) || (TT) ) \d+) \s+ (?P<object> \d+)#x', $object_data, $matches ) )
		   {
				...
				for  ( $i = 0, $count = count ( $code_matches [ 'char' ] ) ; $i  <  $count ; $i += 2 )
				   {
					   ...
					   $this -> DirectMap [ $src ]	=  $this -> CodePointToUtf8 ( $dst ) ;
				    }
		    	    }
		     }
						...
						if  ( $from  !=  $to )
						   {
							...
							$this -> DirectMap [ $from ]	=  $this -> CodePointToUtf8 ( $subst ) ;
					    }
					// Array form :
					...
					// We put them in the DirectMap array, since there is only a one to one correspondance
									...
									$range, $range_match ) )
					   {
						...
						for  ( $i = $from, $count = 0 ; $i  <=  $to ; $i ++, $count ++ )
							$this -> DirectMap [$i] 	=  $this -> CodePointToUtf8 ( hexdec ( $subst [ $count ] ) ) ;
					    }
				   }
			    }
			...
			if  ( $result  !==  false )
			   {
				...
				$this -> DirectMap [ $offset ]	=  $code ;

				return ( $code ) ;
			    }
		    }
			
		// None of the above : return the converted code as a last resort (store it in the direct map before)
